...
4. **🎯 공고 매칭** — 기업 프로필 + 보유 서류 기반 적합도 정렬.
   - **유형 필터 칩**: 💰 자금 지원 / 🏢 입주·공간 / 🎓 교육·멘토링 / 🔬 R&D·기술 / 🌏 판로·글로벌 / 🎯 행사·네트워크 (다중 선택, 카드별 카운트). K-Startup `supt_biz_clsfc` 메타 + 비-K-Startup 은 제목 키워드로 자동 분류.
   - **카드 컬러 배지**: 제목 옆에 유형 배지 (자금=노랑/입주=하늘/교육=보라/R&D=초록/판로=청록/행사=핑크) — 필터 안 걸어도 한눈에 식별.
   - **체크박스 2개 이상 → 비교 모달** (아래 [공고 비교](#-공고-비교-기능) 참고).
